...
if __name__ == "__main__":
    ...
    def getResponse():
        ...
        if chain is not None:
            response = chain.invoke(userQuery + " if question is about appointment, medical history, health facilities, patient information retrieve from the json part of the data")
            return jsonify({'response': response})
        else:
            return jsonify({'error': 'Model is not trained yet'}), 400
    
    @app.route('/trainModel/<string:phoneNumber>')
    def trainModel(phoneNumber):
        global chain, vectorStore  # Ensure we use the global variables
        if phoneNumber is not None:
            command = ["./serialize_data.sh", phoneNumber]
            subprocess.call(command)
            rag_chain, vectorstore = llmPipeline()  # Assuming llmPipeline is defined
            chain = rag_chain
            vectorStore = vectorstore
